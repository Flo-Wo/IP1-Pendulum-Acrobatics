...
from utils.experiments.trajectory import PenaltyTrajectory, ExperimentTrajectory
...
def _build_residual_models(
    ...
    """
    res_factory = ResidualFactory(state, dim_control=experiment.dim_control)
    res_list = []
    if penalty.u_pen is not None:
        res_list.append(
            res_factory.compute_residual(
                "u", "control", penalty.u_pen, dim_residual=experiment.dim_control
            )
        )
