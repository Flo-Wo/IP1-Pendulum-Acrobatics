...
from utils.evaluation.evaluation_setpoint import _prefilter_metadata, _config_table
...
def eval_ddp_precomputed(
    ...
    for model_name in eval_models:
        ...
        model = ConfigWAM(model_name)
        print("x_end_world: ", model.pend_end_world())
        experiment = EvalExperimentSetpoint(
            time_horizon, start_point=model.pend_end_world()
        )
        res = _eval_model_ddp(
            metadata,
            model_name,
...
def _load_single_entry(
    ...
    if angle:
        traj = angles_traj(
            ...
            pin_data,
            q_list,
            frame1_id=pend_end_frame_idx,
            frame2_id=pend_beg_frame_idx,
            radian=radian,
        )
    else:
